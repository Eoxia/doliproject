...
/**
 * \file    doliproject/admin/setup.php
 * \ingroup doliproject
 * \brief   Doliproject setup page.
 */
...
} elseif ($action == 'specimen')
{
	...
	{
		$file = dol_buildpath($reldir."core/modules/doliproject/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file))
		{
			$filefound = 1;
...
} elseif ($action == 'setmod')
{
	...
	$tmpobjectkey = GETPOST('object');
	$constforval = 'DOLIPROJECT_'.strtoupper($tmpobjectkey)."_ADDON";
...
$page_name = "DoliprojectSetup";
llxHeader('', $langs->trans($page_name), '', '', 0, 0, array(), $morecss);
...
dol_fiche_head($head, 'settings', '', -1, "doliproject@doliproject");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("DoliprojectSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false)
					{
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php')
						{
							...
							if ($module->isEnabled())
							{
								...
								$constforvar = 'DOLIPROJECT_'.strtoupper($myTmpObjectKey).'_ADDON';
