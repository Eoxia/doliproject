 ...
 * \ingroup doliproject
 * \brief   DoliProject timesheetdocument config page.
 */
...
if ($action == 'setdoc') {
	...
	$constforval = 'DOLIPROJECT_'.strtoupper($type)."_ADDON";
...
print dol_get_fiche_head($head, 'timesheetdocument', '', -1, "doliproject@doliproject");
...
foreach ($types as $type => $documentType) {
	...
	$trad = 'DoliProject' . $type . 'DocumentNumberingModule';
	...
	clearstatcache();
	$dir = dol_buildpath("/custom/doliproject/core/modules/doliproject/".$documentType."/");
	if (is_dir($dir)) {
		$handle = opendir($dir);
				...
				if (!is_dir($dir.$file) || (substr($file, 0, 1) <> '.' && substr($file, 0, 3) <> 'CVS')) {
					...
					if (!class_exists($classname) && is_readable($dir.$filebis) && (preg_match('/mod_/', $filebis) || preg_match('/mod_/', $classname)) && substr($filebis, dol_strlen($filebis) - 3, 3) == 'php') {
						...
						if ($module->isEnabled()) {
							...
							$confType = 'DOLIPROJECT_' . strtoupper($documentType) . '_ADDON';
