...
dol_include_once('/doliproject/class/timesheet.class.php');
dol_include_once('/doliproject/lib/doliproject_timesheet.lib.php');

// Load translation files required by the page
$langs->loadLangs(array("doliproject@doliproject", "companies"));
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->doliproject->multidir_output[!empty($object->entity) ? $object->entity : $conf->entity]."/".$object->id;
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->rights->doliproject->timesheet->write;
...
if ($id > 0 || !empty($ref)) {
	...
	print dol_get_fiche_head($head, 'note', $langs->trans("TimeSheet"), -1, 'doliproject@doliproject');

	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/doliproject/view/timesheet/timesheet_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
