...
$permissiontoread   = $user->rights->doliproject->$object_type->read;
$permissiontoadd    = $user->rights->doliproject->$object_type->write;
$permissiontodelete = $user->rights->doliproject->$object_type->delete;
...
if (empty($backtopage) || ($cancel && empty($id))) {
	if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
		$backtopage = dol_buildpath('/doliproject/view/'. $object->element .'/' . $object->element .'_attendants.php', 1) . '?id=' . ($object->id > 0 ? $object->id : '__ID__');
	}
}
...
if ($action == 'send') {
	...
	if ( ! $error) {
		...
		if (dol_strlen($sendto) && ( ! empty($conf->global->MAIN_MAIL_EMAIL_FROM))) {
			...
			$url  = dol_buildpath('/custom/doliproject/public/signature/add_signature.php?track_id=' . $signatory->signature_url  . '&type=' . $object->element, 3);
...
$help_url = '';
$morejs   = array("/doliproject/js/signature-pad.min.js", "/doliproject/js/doliproject.js.php");
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	print dol_get_fiche_head($head, 'attendants', $langs->trans("TimeSheet"), -1, 'doliproject@doliproject');
	...
	<?php if ( $object->status == $object::STATUS_DRAFT ) : ?>
		<div class="wpeo-notice notice-warning">
			...
			<a class="butAction" style="width = 100%;margin-right:0" href="<?php echo DOL_URL_ROOT ?>/custom/doliproject/view/<?php echo $object->element ?>/<?php echo $object->element ?>_card.php?id=<?php echo $id ?>"><?php echo $langs->trans("GoToValidate") ?></a>;
		</div>
	<?php endif; ?>
	...
	if (is_array($society_intervenants) && ! empty($society_intervenants) && $society_intervenants > 0) {
		foreach ($society_intervenants as $element) {
			...
			if ($object->status == $object::STATUS_VALIDATED) {
				$signatureUrl = dol_buildpath('/custom/doliproject/public/signature/add_signature.php?track_id=' . $element->signature_url  . '&type=' . $object->element, 3);
