...
class ActionsDoliproject
{
	...
	public function doActions($parameters, &$object, &$action, $hookmanager)
	{
		...
		// echo $object->id . '<br>';
		// echo $object->ref . '<br>';
		// echo $object->lines[0]->fk_facture . '<br>';
	...
	public function addMoreMassActions($parameters, &$object, &$action, $hookmanager)
	{
		...
		if (in_array($parameters['currentcontext'], array('somecontext1', 'somecontext2')))		// do something only for the context 'somecontext1' or 'somecontext2'
		{
			$this->resprints = '<option value="0"'.($disabled ? ' disabled="disabled"' : '').'>'.$langs->trans("DoliprojectMassAction").'</option>';
		}
	...
	public function loadDataForCustomReports($parameters, &$action, $hookmanager)
	{
		...
		if ($parameters['tabfamily'] == 'doliproject') {
			$head[$h][0] = dol_buildpath('/module/index.php', 1);
			$head[$h][1] = $langs->trans("Home");
			$head[$h][2] = 'home';
			$h++;

			$this->results['title'] = $langs->trans("Doliproject");
			$this->results['picto'] = 'doliproject@doliproject';
		}
	...
	public function restrictedArea($parameters, &$action, $hookmanager)
	{
		...
		if ($parameters['features'] == 'myobject') {
			if ($user->rights->doliproject->myobject->read) {
	...
	public function addMoreActionsButtons($parameters, &$object, &$action, $hookmanager)
	{
		...
		if (in_array($parameters['currentcontext'], array('invoicecard')))		// do something only for the context 'somecontext1' or 'somecontext2'
		{
			...
			while ($data_cut = $data->fetch()) {
				if ($data_cut['fk_facture_name'] == $object->id) {
					$error_button = 1;
				}
			}
			if ($error_button != 1) {
